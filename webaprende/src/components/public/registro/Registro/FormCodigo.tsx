  ...
  const validarCodigo = async (values: Values): Promise<void> => {
    ...
    try {
      const respuesta = await axios.post(
        `${Global.url}/validarCodigo`,
        data
      )
      if (respuesta.data.status === 'success') {
        Swal.fire(
          'Cuenta verificada correctamente',
          'Se envio un correo con tus accesos',
          'success'
        )
        navigate('/login')
      } else {
    ...
    <>
      ...
      <div className="login__body">
        <div className="login__body__main">
          <h1>Codigo de Validación</h1>
          <div className="login__body__main__form">
            <form action="" onSubmit={handleSubmit}>
              <div className="flex flex-col md:flex-row gap-4">
                <div className="input-group relative">
                  <label className="label">Codigo de validación</label>
                  <input
                    autoComplete=""
                    id="Email"
                    className="input"
              ...
              <div className="flex flex-col gap-4">
                <div className="my-0">
                  {validacon == 1
                    ? (
                    <p className="text-red-500 text-2xl">Código inválido</p>
                      )
                    : validacon == 2
                      ? (
                    <p className="text-red-500 text-2xl">Código inválido</p>
