...
const CardCurso = ({ curso, examenes }: { curso: productosValues, examenes: valuesExamenesEntrada[] }): JSX.Element => {
        ...
        <div className="bg-[#6363a24d] px-10 py-8 rounded-xl flex justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-12">
                {/* <span className="flex items-center gap-2">
                    <HiFire className="text-[#8282ff] text-6xl"/>
                    <p className="text-white text-3xl">4.7</p>
                </span> */}
                <Link to={`curso/${curso.id}-${formatearURL(curso.nombre)}`} className="block px-8 py-4 bg-[#8282ff] hover: text-2xl text-white rounded-md">Ver curso</Link>
            </div>
        </div>
    </>
  )
